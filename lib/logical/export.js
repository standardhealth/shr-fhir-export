...
class ModelsExporter {
  ...
  addElement(model, value, isValue=false) {
    ...
    if (value instanceof mdls.TBD) {
      ...
      if (description) {
        ...
        el.short = value.effectiveIdentifier.name;
        el.definition = value.effectiveIdentifier.name;
      }
    }
    // Set the type
  ...
  applyConstraints(model, value, el) {
    ...
    this.preprocessConstraints(value, el);
  ...
  preprocessConstraints(value, el) {
    this.preprocessNestedTypeConstraints(value, el);
    this.preprocessIncludesTypeConstraints(value, el);
  }

  preprocessNestedTypeConstraints(value, el) {
    // SPECIAL HANDLING FOR NESTED TYPE CONSTRAINTS:
    // If the value has nested type constraints, then we must make it a backbone element instead.  This is
    ...
    if (value.constraintsFilter.type.child.hasConstraints) {
      ...
      for (const c of child) {
        ...
        // Set the type
        const tcValue = currentEl.type[0].code === 'Reference' ? new mdls.RefValue(c.isA) : new mdls.IdentifiableValue(c.isA);
        currentEl.type = this.toTypeArray(tcValue);

        // Adjust the aliases / id / path as necessary
        const shortName = common.shortID(c.isA, true);
        ...
        const description = this.getDescription(c.isA);
        if (description) {
          currentEl.short = this.shortDescription(description);
          currentEl.definition = description;
        } else {
          currentEl.short = c.isA.name;
          currentEl.definition = c.isA.name;
        }
        // Set the codes if this element has Concepts defined
        const codes = this.getConcepts(c.isA);
        if (codes && codes.length) {
  ...
  preprocessIncludesTypeConstraints(value, el) {
    ...
    if (value.constraintsFilter.includesType.hasConstraints) {
      ...
      const own = value.constraintsFilter.includesType.own.constraints;
      const child = value.constraintsFilter.includesType.child.constraints;
  ...
  applyIncludesTypeConstraint(model, value, el, constraint) {
    ...
    if (el.min === 0 && constraint.card.min > 0) {
      el.min = 1;
    }
    ...
    if (description) {
      ...
      child.short = constraint.isA.name;
      child.definition = constraint.isA.name;
    }
    // Set the type
