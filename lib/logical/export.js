...
class ModelsExporter {
  ...
  applyConstraints(model, value, el) {
    for (const c of value.constraintsFilter.constraints) {
  ...
  applyValueSetConstraint(model, value, el, constraint) {
    ...
    el.applyBinding(vsURI, strength, constraint.path.map(id => common.shortID(id, true)).join('.'), this.resolve.bind(this));
  }
