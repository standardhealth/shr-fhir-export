...
class ModelsExporter {
  ...
  applyIncludesCodeConstraint(model, value, el, constraint) {
    ...
    if (elToConstrain.constraint == null) {
      elToConstrain.constraint = [];
    }

    const invariant = {
      key: `${elToConstrain.path.split('.').pop()}-${elToConstrain.constraint.length + 1}`,
      severity: 'error',
      human: '',
      expression: ''
    };
    ...
    if (elToConstrain.type.some(t => t.code === 'CodeableConcept')) {
      if (code.system) {
        invariant.human = `There must exist a coding with system '${code.system}' and code '${code.code}'.`;
        invariant.expression = `coding.where(system = '${code.system}' and code = '${code.code}').exists()`;
      } else {
        invariant.human = `There must exist a coding with and code '${code.code}'.`;
        invariant.expression = `coding.where(code = '${code.code}').exists()`;
      }
    } else if (elToConstrain.type.some(t => t.code === 'Coding')) {
      if (code.system) {
        invariant.human = `There must exist a pairing of system '${code.system}' and code '${code.code}'.`;
        invariant.expression = `where(system = '${code.system}' and code = '${code.code}').exists()`;
      } else {
        invariant.human = `There must exist a code '${code.code}'.`;
        invariant.expression = `where(code = '${code.code}').exists()`;
      }
    } else if (elToConstrain.type.some(t => t.code === 'code')) {
      invariant.human = `There must exist a value '${code.code}'.`;
      invariant.expression = `where($this = '${code.code}').exists()`;
    } else {
      // Not something we can fix a code on
      return;
    }

    elToConstrain.constraint.push(invariant);
  }
