...
class ModelsExporter {
  ...
  applyIncludesCodeConstraint(model, value, el, constraint) {
    ...
    if (elToConstrain.type.some(t => t.code === 'CodeableConcept')) {
      if (code.system) {
        invariant.human = `There must exist a ${prefixPath}coding with system '${code.system}' and code '${code.code}'.`;
        invariant.expression = `${prefixPath}coding.where(system = '${code.system}' and code = '${code.code}').exists()`;
      } else {
        invariant.human = `There must exist a ${prefixPath}coding with and code '${code.code}'.`;
        invariant.expression = `${prefixPath}coding.where(code = '${code.code}').exists()`;
      }
    } else if (elToConstrain.type.some(t => t.code === 'Coding')) {
      if (code.system) {
        invariant.human = `There must exist a pairing of ${prefixPath}system '${code.system}' and ${prefixPath}code '${code.code}'.`;
        invariant.expression = `${prefixPath}where(system = '${code.system}' and code = '${code.code}').exists()`;
      } else {
        invariant.human = `There must exist a ${prefixPath}code '${code.code}'.`;
      ...
      invariant.human = `There must exist a value of '${code.code}'${suffix}`;
