...
function exportToFHIRLogicalModels(specifications, configuration) {
  const exporter = new FHIRModelsExporter(specifications, configuration);
  return exporter.export();
}

class FHIRModelsExporter {
  ...
  export() {
    ...
    return {
      models: Array.from(this._modelsMap.values()).map(m => m.toJSON())
    };
  }
  ...
  exportModel(def) {
    ...
    try {
      ...
      model.id = model.type = common.fhirID(def.identifier);
...
module.exports = {exportToFHIRLogicalModels, FHIRModelsExporter, setLogger, TARGET, MODELS_INFO: mdls.MODELS_INFO};
