...
class ModelsExporter {
  ...
  export() {
    ...
    for (const element of this._specs.dataElements.all) {
      try {
        ...
        logger.error({errorText : e.stack }, '13082' );
      }
    }
  ...
  exportModel(def) {
    ...
    // 03023, 'Start exporting element logical model',,
    logger.debug('03023');
    try {
      ...
      // 03024, 'Done exporting element logical model',,
      logger.debug('03024');
      logger = lastLogger;
    }
  }
  ...
  applyConstraints(model, value, el) {
    ...
    for (const c of value.constraintsFilter.subset.constraints) {
      this.applySubsetConstraint(model, value, el, c);
    }
   ...
   * associated FHIR path would be in a FHIR StructureDefinition logical model.
  ...
  shrPathToFhirPath(value, path) {
    ...
    return path.map(id => common.shortID(id, true)).join('.');
  }
  ...
  applyTypeConstraint(model, value, el, constraint) {
    ...
    const tcValue = new mdls.IdentifiableValue(newType);
   ...
   * @param {Object} constraint - the constraint to apply
   */
  applySubsetConstraint(model, value, el, constraint) {
    ...
    // Set the types
    const newTypes = constraint.subsetList;
    ...
    currentEl.type = currentEl.type.filter(t => subsetTypes.some(subsetType => subsetType.code === t.code));
  }
  ...
  applyIncludesTypeConstraint(model, value, el, constraint) {
    ...
    // Set the type
    const newType = constraint.isA;
    const itcValue = new mdls.IdentifiableValue(newType);
...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  toSingleType(value) {
    ...
    if (id.equals(CONCEPT_ID)) {
      // In a logical model, always use Coding for concept types
      return { code: 'Coding' };
    } else if (id.isPrimitive) {
      return { code: id.name };
    }
    // If it's an entry, make it a reference
    const de = this._specs.dataElements.findByIdentifier(id);
    if (de.isEntry) {
  ...
  toTypeArray(value) {
    if (value instanceof mdls.TBD) {
      ...
      nonTBDs.map(o => this.toSingleType(o)).forEach(t => {
        if (typeMap.has(t.code)) {
          const t2 = typeMap.get(t.code);
          if (t.targetProfile && t.targetProfile.length > 0) {
            if (t2.targetProfile == null) {
              t2.targetProfile = [];
            }
