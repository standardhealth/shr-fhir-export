 ...
 * @see {@link http://hl7.org/fhir/R4/elementdefinition.html|FHIR ElementDefinition}
 */
class ElementDefinition {
  ...
  /**
   * @returns {boolean} sliceIsConstraining
   */
  get sliceIsConstraining() { return this._sliceIsConstraining; }
  /**
   * @param {boolean} sliceIsConstraining
   */
  set sliceIsConstraining(sliceIsConstraining) { this._sliceIsConstraining = sliceIsConstraining; }
  ...
  /**
   * @returns {string} isModifierReason
   */
  get isModifierReason() { return this._isModifierReason; }
  /**
   * @param {string} isModifierReason
   */
  set isModifierReason(isModifierReason) { this._isModifierReason = isModifierReason; }
  ...
  /**
   * @returns {{strength: string, description?: string, valueSet?: string}} binding
   */
  get binding() { return this._binding; }
  /**
   * @param {{strength: string, description?: string, valueSet?: string}} binding
   */
  ...
  calculateDiff() {
    ...
    diff._structDef = this._structDef;
   ...
   * @see {@link http://hl7.org/fhir/R4/profiling.html#slicing|FHIR Profiles: Slicing}
  ...
  newSlice(name, type) {
    ...
    slice.isModifierReason = this.isModifierReason;
  ...
  toJSON() {
    ...
    if (this.structDef && this.structDef.fhirVersion === '3.0.1') {
      ...
      if (j.type) {
        const r3Types = [];
        // Need to convert each item in profile or targetProfile arrays to a single type w/ single profile/targetProfile
        j.type.forEach(t => {
          if (t.profile && t.profile.length > 0) {
            t.profile.forEach(p => {
              const t2 = cloneDeep(t);
              t2.profile = p;
              r3Types.push(t2);
            });
          } else if (t.targetProfile && t.targetProfile.length > 0) {
            t.targetProfile.forEach(p => {
              const t2 = cloneDeep(t);
              t2.targetProfile = p;
              r3Types.push(t2);
            });
      ...
      // binding.valueSet --> binding.valueSet[x]
      if (j.binding && j.binding.valueSet) {
        j.binding.valueSetReference = { reference: j.binding.valueSet };
        delete j.binding.valueSet;
      }
    }
  ...
  static fromJSON(json) {
    ...
    // type.profile / type.targetProfile --> type.profile[] / type.targetProfile[]
    if (json.type && json.type.some(t => typeof t.profile === 'string' || typeof t.targetProfile === 'string')) {
      const r4Types = new Map();
      json.type.forEach(t => {
        if (r4Types.has(t.code)) {
          const t2 = r4Types.get(t.code);
          if (t.profile && t2.profile.indexOf(t.profile) === -1) {
            t2.profile.push(t.profile);
          } else if (t.targetProfile && t2.targetProfile.indexOf(t.targetProfile) === -1) {
            t2.targetProfile.push(t.targetProfile);
          }
        } else {
          const t2 = cloneDeep(t);
          if (t.profile) {
            t2.profile = [t.profile];
          } else if (t.targetProfile) {
            t2.targetProfile = [t.targetProfile];
          }
          r4Types.set(t2.code, t2);
        }
      });
      ed.type = Array.from(r4Types.values());
    }

    // binding.valueSet[x] --> binding.valueSet
    if (json.binding) {
      if (json.binding.valueSetReference) {
        ed.binding.valueSet = json.binding.valueSetReference.reference;
        // @ts-ignore
        delete ed.binding.valueSetReference;
      } else if (json.binding.valueSetUri) {
        ed.binding.valueSet = json.binding.valueSetUri;
        // @ts-ignore
        delete ed.binding.valueSetUri;
      }
    }
 ...
 */
const PROPS = [ 'id', 'extension', 'modifierExtension', 'path', 'representation', 'sliceName', 'sliceIsConstraining',
  'label', 'code', 'slicing', 'short', 'definition', 'comment', 'requirements', 'alias', 'min', 'max', 'base',
