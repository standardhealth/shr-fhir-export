...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  processMappingRules(map, profile) {
    ...
    const slicedTargetPaths = new Map();
    const needsSliceTargetPaths = new Map();
    let currentSlicablePath = '';
    for (const rule of map.rules) {
      if (rule instanceof mdls.FieldMappingRule) {
        const [target, commands] = extractTargetAndCommands(rule.target);
        if (currentSlicablePath != '' && !target.startsWith(currentSlicablePath)) {
          currentSlicablePath = '';
        }
        if (target.startsWith('http://') || target.startsWith('https://')) {
          continue;
        }
        if (mappedTargetPaths.has(target)) {
          if (currentSlicablePath == '') {
            if (!slicedTargetPaths.has(target)) {
              needsSliceTargetPaths.set(target, true);
            }
            currentSlicablePath = target;
          }
          continue;
        }
        mappedTargetPaths.set(target, true);
        if (typeof commands !== 'undefined' && commands.some(c => c.startsWith('slice'))) {
          slicedTargetPaths.set(target);
        }
      }
    }
    if (!disableError.needsSlice) {
      for (const t of needsSliceTargetPaths.keys()) {
        this._errors.push(new common.FHIRExportError(`${profile.id}: Contains mappings to ${t} that should be sliced`));
      }
    }
  ...
  processFieldToFieldMappingRule(map, rule, profile) {
    ...
    const [target] = extractTargetAndCommands(rule.target);
    ...
    if (typeof ss === 'undefined') {
      ...
      const sses = this.getSnapshotElements(profile, target);
      if (sses.length == 0) {
        if (!disableError.invalidTarget) this._errors.push(new common.FHIRExportError(`${profile.id}: Cannot apply field mapping to ${profile.type}.  Invalid target path: ${target}`));
      }
      return;
    }
    ...
    let df = common.getDifferentialElement(profile, target);
...
function extractTargetAndCommands(target) {
  const matches = /([^\s\(]+)(\s+\(([^\)]+)\))?/.exec(target);
  if (matches == null || typeof matches[1] === 'undefined') {
    return [];
  }
  const result = [matches[1]];
  if (typeof matches[3] !== 'undefined') {
    result.push (matches[3].split(';').map(m => m.trim()));
  }
  return result;
}
