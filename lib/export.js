...
class FHIRExporter {
  ...
  export() {
    ...
    for (const [text, elements] of reverseMap) {
      logger.warn('Unmapped fields in [ %s ]:\n%s', elements.join(', '), text);
    }
...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  conversionIsAllowed(sourceIdentifier, targetTypes) {
    ...
    if (Array.isArray(allowedConversions[fqn])) {
      const allowed = allowedConversions[fqn].some(a => {
        return targetTypes.some(t => t.code == a || t.profile == `http://hl7.org/fhir/StructureDefinition/${a}`
          || (t.code == 'Reference' && t.targetProfile == `http://hl7.org/fhir/StructureDefinition/${a}`));
      });
      if (allowed) {
        return true;
      }
      ...
      if (sourceEl.value instanceof mdls.IdentifiableValue) {
        ...
        for (const opt of sourceEl.value.aggregateOptions) {
          if (opt instanceof mdls.IdentifiableValue) {
            ...
            if (optMatchedPaths) {
              matchedPaths.push(...optMatchedPaths);
  ...
  applyOwnValueSetConstraints(sourceValue, profile, snapshotEl, differentialEl) {
    ...
    if (vsConstraints.length > 0) {
      const vsURI = vsConstraints[0].valueSet;
  ...
  getSnapshotElementForFieldTarget(profile, fieldTarget) {
    ...
    if (fieldTarget.hasSliceOnCommand()) {
      ...
      common.addSlicingToBaseElement(baseElement, null, 'value', fieldTarget.findSliceOnCommand().value);
  ...
  checkMappedAndConstrained(profile, path) {
    ...
    if (this.elementTypeNotChanged(profile, codeEl)) {
      ...
      if (!profile.id.endsWith(`-${profile.type}`)) {
        logger.warn('The \'%s\' property is not bound to a value set or fixed to a code. This property is core to the target resource and usually should be constrained.', path);
      }
    }
  }
