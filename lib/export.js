...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  getDescription(identifier, defaultToName = false) {
    const def = this._specs.dataElements.findByIdentifier(identifier);
    let description;
    if (def) {
      description = def.description;
    }
    if (defaultToName && (typeof description === 'undefined' || description == null || description == '')) {
...
*/
  ...
  addExtension(profile, identifier, card, extURL) {
    ...
    ssEl.definition = this.getDescription(identifier, true);
    delete(ssEl.short);
