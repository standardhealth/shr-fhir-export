...
class FHIRExporter {
  ...
  mappingToProfile(map) {
    ...
    try {
      ...
      // Modify the differential elements as appropriate
      if (def.id === def.type) {
        // This signals that the core definition is a FHIR resource (not profile), so clear differentials and add root
        ...
        // This profile is based on another profile, so keep differentials, but modify their IDs
        for (const dfEl of profile.differential.element) {
          dfEl.id = dfEl.id.replace(new RegExp(`^${profile.type}(:[^\.]+)?`), `${profile.type}:${profileID}`);
        }
        const rootDF = profile.differential.element[0];
        rootDF.short = rootSS.short;
        rootDF.definition = rootSS.definition;
        // Some implementation guides put a slice name on the root element, but this is not necessary
        delete(rootDF.sliceName);
      }
...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  addExtension(def, profile, sourcePath, extURL) {
    ...
    if (typeof ssEl === 'undefined') {
      ...
      ssEl.id = ssEl.id.replace(/:[^\.]+$/, ':' + common.shortID(identifier));
      ssEl.sliceName = common.shortID(identifier);
      ...
      if (sourceValue.effectiveCard.fitsWithinCardinalityOf(targetCard)) {
        setCardinalityOnFHIRElements(sourceValue.effectiveCard, ssEl);
      } else {
        logger.error('Cannot constrain cardinality from %s to %s', targetCard.toString(), sourceValue.effectiveCard.toString());
      }
      if (isModifier || typeof ssEl.mustSupport === 'undefined') {
        ssEl.mustSupport = isModifier;
      }
      if (isModifier || typeof ssEl.isModifier === 'undefined') {
        ssEl.isModifier = isModifier;
      }
      if (typeof ssEl.isSummary === 'undefined') {
        ssEl.isSummary = false;
      }
      delete(ssEl.short);
    ...
    if (typeof dfEl === 'undefined') {
      ...
      // The differential exists, but we should re-set any values that might have changed
      dfEl.id = ssEl.id;
      dfEl.sliceName = ssEl.sliceName;
      dfEl.definition = ssEl.definition;
      dfEl.min = ssEl.min;
      dfEl.max = ssEl.max;
      dfEl.mustSupport = ssEl.mustSupport;
      dfEl.isModifier = ssEl.isModifier;
      dfEl.isSummary = ssEl.isSummary;
    }
