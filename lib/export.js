...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  qaCheckMappedAndConstrained(profile, path) {
    ...
    if (this.elementTypeNotChanged(codeEl)) {
      qa.propertyNotMapped(`${profile.type}.${path}`, profile.id, codeEl.path);
    } else if (this.elementTypeUnconstrainedCode(codeEl)) {
      qa.codeNotConstrained(`${profile.type}.${path}`, profile.id, codeEl.path);
    }
  }

  elementTypeNotChanged(el) {
    const rEl = this.getOriginalElement(el);
    return simpleJSONEqual(el.type, rEl.type);
  }

  elementTypeUnconstrainedCode(el) {
    return this.elementTypeIsCodishOrQuantity(el) && this.codeNotConstrained(el);
  }

  elementTypeIsCodishOrQuantity(el) {
    return el.type.some(t => ['code', 'Coding', 'CodeableConcept', 'Quantity'].includes(t.code));
  }

  codeNotConstrained(el) {
    const rEl = this.getOriginalElement(el);
