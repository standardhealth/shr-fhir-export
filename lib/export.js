...
class FHIRExporter {
  constructor(specifications, configuration = {}) {
    ...
    // Find the target (TODO: Support multiple targets)
    this._target = this._specs.maps.targets.length ? this._specs.maps.targets[0] : 'FHIR_STU_3';
  ...
  */
  ...
  addExtension(def, profile, sourcePath, extURL) {
    ...
    let ssEl = profile.snapshot.element.find(e => {
      return e.path === `${MVH.sdType(profile)}.${extType}` && e.type.length === 1 && e.type[0].profile === extURL;
    });
...
function isTargetBasedOn(target, baseTarget, targetSpec) {
  ...
  else if ((tID !== tType && isTargetBasedOn(tType, bID, this._target)) || (tURL !== tBaseDef && isTargetBasedOn(tBaseDef, bURL, this._target))) {
    return true;
  }
