...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  processValueToFieldType(map, sourcePath, sourceValue, profile, snapshotEl, differentialEl) {
    ...
    if (sourceEl && sourceEl.value) {
      ...
      if (sourceElValue instanceof mdls.ChoiceValue) {
        ...
        // If it has subset constraints, we need to remodel the choice
        const subCst = sourceValue.constraintsFilter.subset.constraints;
        if (subCst.length === 1) {
          // Only keep options that are listed in the constraint
          const filteredOptions = sourceElValue.options.filter(opt => {
            return subCst[0].subsetList.some(type => opt.identifier.equals(type));
          });
          sourceElValue.options = filteredOptions;
        }
      }
  ...
  findMatchingType(sourceValue, targetTypes, sourceIdentifier=null) {
    if (sourceIdentifier != null) {
      sourceIdentifier = common.choiceFriendlyEffectiveIdentifier(sourceValue, sourceIdentifier);
    } else {
      sourceIdentifier = common.choiceFriendlyEffectiveIdentifier(sourceValue);
    }
    ...
    if (sourceValue.constraintsFilter.child.hasConstraints) {
      ...
      if (isExtension) {
        ...
        for (const csts of pathToConstraintMap.values()) {
          ...
          this.applyOwnSubsetConstraintsOnExtension(childSourceValue, profile, element, diffElement);
  ...
  applyOwnSubsetConstraintsOnExtension(sourceValue, profile, snapshotEl, differentialEl) {
    const subsetConstraints = sourceValue.constraintsFilter.own.subset.constraints;

    if (subsetConstraints.length > 0) {
      [snapshotEl, differentialEl] = this.findConstrainableElement(sourceValue, profile, snapshotEl, differentialEl);

      const sourceIdentifiers = common.subsetEffectiveIdentifiers(sourceValue);
      const targetTypes = snapshotEl.type;

      // Note that this code is very similar to code in applyOwnTypeConstraintsOnExtension, but this code
      // loops over multiple possible sourceIdentifiers in a loop instead of acting on only one
      for (const sourceIdentifier of sourceIdentifiers) {
        ...
        // Check if the source field has a mapping to a FHIR profile.  If so, and it matches target, apply the profile to the target
        const originalTargetTypes = common.cloneJSON(targetTypes);
        const matchedType = this.findMatchingType(sourceValue, targetTypes, sourceIdentifier);
        if (typeof matchedType !== 'undefined') {
          // We got a match!
          const sourceProfile = this.lookupProfile(sourceIdentifier, true, false);
          // Check to see if this is trying to map a different element than the one that was previously mapped.
