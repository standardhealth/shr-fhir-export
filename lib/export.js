  ...
  processFieldToFieldCardinality(map, rule, profile, snapshotEl, differentialEl) {
    ...
    if (targetPath.length == 1) {
      ...
      if (aggSourceCard.fitsWithinCardinalityOf(targetCard)) {
        ...
        this._errors.push(new FHIRExportError(`${profile.id}: Incompatible cardinality since ${sourcePathToString(rule.sourcePath, true)} (${srcAggStmt}${aggSourceCard.toString()}) does not fit in ${profile.type}.${targetPath} (${tgtAggStmt}${targetCard})`));
        return;
      }
    } else {
      ...
      if (aggSourceCard.equals(aggTargetCard)) {
        ...
        if (!isMatch) {
          this._errors.push(new FHIRExportError(`${profile.id}: Cannot constrain ${profile.type}.${rule.target} to ${aggSourceCard.toString()} (from ${sourcePathToString(rule.sourcePath, true)}) because elements of its path are unconstrained, so cardinality placement is ambiguous.`));
          return;
        }
        ...
        if (sMin == aggTargetCard.min) {
          ...
          this._errors.push(new FHIRExportError(`${profile.id}: Cannot constrain ${profile.type}.${rule.target} to ${aggSourceCard.toString()} (from ${sourcePathToString(rule.sourcePath, true)}) (from ${sourcePathToString(rule.sourcePath, true)}) because there is no tail cardinality min that can get us there.`));
          return;
        }
        ...
        if (sMax == aggTargetCard.max) {
          ...
          this._errors.push(new FHIRExportError(`${profile.id}: Cannot constrain ${profile.type}.${rule.target} to ${aggSourceCard.toString()} (from ${sourcePathToString(rule.sourcePath, true)}) because there is no tail cardinality max that can get us there.`));
          return;
        }
        ...
        if (magicCard.fitsWithinCardinalityOf(targetCard)) {
          ...
          this._errors.push(new FHIRExportError(`${profile.id}: Cannot constrain ${profile.type}.${rule.target} to ${aggSourceCard.toString()} (from ${sourcePathToString(rule.sourcePath, true)}) because there is no tail cardinality that can get us there.`));
          return;
        }
      }
    }
