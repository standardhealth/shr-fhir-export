...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  processCardinalityMappingRule(map, rule, profile) {
    ...
    if (rule.cardinality.fitsWithinCardinalityOf(targetCard)) {
      ...
      logger.error('Cannot constrain cardinality from %s to %s', targetCard.toString(), rule.cardinality.toString());
    }
  ...
  fixCodeOnElement(codeType, profile, snapshotEl, differentialEl, code, usePattern=true) {
    ...
    // TODO: Change the method of fixing codes to use system and code subpaths
    ...
    // Determine if the snapshot element already has fixed code-like values

    // First check for it at the higher Code/Coding/CodeableConcept/Quantity level
    ...
    // It's also possible that it's applied via a slice and/or separate paths for code & system, so check for that
    switch (codeType) {
    case 'shr.core.Coding':
    case 'Coding':
    case 'shr.core.Quantity':
    case 'Quantity':
      if (this.skipFixedCodeForCodingOrQuantitySubPaths(profile, snapshotEl, code.system, code.code)) {
        return;
      }
      break;
    case 'shr.core.CodeableConcept':
    case 'CodeableConcept':
      // The system/code pair will be in a slice on coding
      var codingSlices = profile.snapshot.element.filter(e => e.id.startsWith(snapshotEl.id) && e.path === `${snapshotEl.path}.coding` && e.sliceName);
      for (const codingSlice of codingSlices) {
        if (this.skipFixedCodeForCodingOrQuantitySubPaths(profile, codingSlice, code.system, code.code)) {
          return;
        }
      }
      break;
  ...
  skipFixedCodeForCodingOrQuantitySubPaths(profile, codingEl, system, code) {
    const systemEl = profile.snapshot.element.find(e => e.id.startsWith(codingEl.id) && e.path === `${codingEl.path}.system`);
    const codeEl = profile.snapshot.element.find(e => e.id.startsWith(codingEl.id) && e.path === `${codingEl.path}.code`);
    if (systemEl || codeEl) {
      const currentSystem = systemEl ? systemEl.fixedUri : '<none>';
      const currentCode = codeEl ? codeEl.fixedCode : '<none>';
      if ((currentSystem && currentSystem !== system) || (currentCode && currentCode !== code)) {
        logger.error('Cannot override code constraint from %s|%s to %s|%s', currentSystem, currentCode, system, code);
        // Return true since we shouldn't re-fix an already fixed (but different) code
        return true;
      }
      // If both the current system and code exist, return true since they're already fixed.
      // Otherwise the system or code still needs to be fixed.
      return currentSystem & currentCode;
    }
  }
