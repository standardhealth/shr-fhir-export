...
class FHIRExporter {
  ...
  export() {
    ...
    // TODO: Add support for models in DSTU2 and R4
  ...
  processFieldToFieldType(map, def, rule, profile, snapshotEl, differentialEl) {
    ...
    if (originalShort && originalShort.indexOf('|') !== -1) {
      ...
      if (sourceIdentifier && snapshotEl.definition === originalDefinition) {
        snapshotEl.definition = this.getDescription(sourceIdentifier, sourceIdentifier.name);
        if (snapshotEl.definition !== originalDefinition) {
          // It really only makes sense to change the short if we changed the long definition
          // since our short is inferior (just truncated version of description).  That's why
          // this logic is inside the above if statement.
          snapshotEl.short = this.getDescriptionAsShort(sourceIdentifier, sourceIdentifier.name);
          if (snapshotEl.short !== originalShort) {
            differentialEl.short = snapshotEl.short;
          }
          // Update differential definition last to maintain order of short, definition
          differentialEl.definition = snapshotEl.definition;
        }
      }
    }
