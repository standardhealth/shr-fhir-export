...
class FHIRExporter {
  ...
  export() {
    ...
    // Iterate through the primary elements and do the mappings
    const filter = this._config.igPrimarySelectionStrategy.filter;
    const strategy = this._config.igPrimarySelectionStrategy.strategy;
    const primary = this._config.igPrimarySelectionStrategy.primary;
    for (const element of this._specs.dataElements.all) {
      if ((!filter)
      || ((strategy === "element") && (primary.includes(element.identifier.name)))
    ...
    this._valueSetExporter.export(profiles);
    this._codeSystemExporter.export(profiles, this._valueSetExporter.valueSets);
