  ...
  export() {
    ...
    for (const entry of this._specs.dataElements.entries) {
      ...
      if (typeof map === 'undefined') {
        // console.log('Adding default mapping to Basic for', entry.identifier.fqn);
...
`<div xmlns="http://www.w3.org/1999/xhtml">
    ...
    if (targetPath.length == 1 || (targetPath.length == 2 && targetPath[0].endsWith('[x]')) ) {
      ...
      if (aggSourceCard.equals(aggTargetCard)) {
        ...
        if (sMin == aggTargetCard.min) {
          ...
          if (!disableError.impossibleAggregateCardMapping) this._errors.push(new FHIRExportError(`${profile.id}: Cannot constrain ${profile.type}.${rule.target} to ${aggSourceCard.toString()} (from ${sourcePathToString(rule.sourcePath, true)}) because there is no tail cardinality min that can get us there.`));
          return;
        }
  ...
  applyConstraints(sourceValue, profile, snapshotEl, differentialEl) {
    ...
    this.applyIncludesCodeConstraints(sourceValue, profile, snapshotEl, differentialEl);
    this.applyBooleanConstraints(sourceValue, profile, snapshotEl, differentialEl);
  }
  ...
  applyCodeConstraints(sourceValue, profile, snapshotEl, differentialEl) {
    ...
    if (codeConstraints.length > 0) {
      ...
      if (typeof property === 'undefined') {
        this._errors.push(new FHIRExportError(`${profile.id}: Can't fix code on ${snapshotEl.id} because source value isn't code-like: ${sourceValue.identifier}.  This should never happen and is probably a bug in the tool.`));
        return;
      }
      ...
      for (let key of ['fixedCode', 'fixedCoding', 'fixedCodeableConcept', 'patternCode', 'patternCoding', 'patternCodeableConcept']) {
        if (snapshotEl.hasOwnProperty(key)) {
          ...
          if (!disableError.overrideFixedValue) this._errors.push(new FHIRExportError(`${profile.id}: Cannot override fixed code on ${snapshotEl.id} from ${JSON.stringify(snapshotEl[key])} to ${JSON.stringify(value)}.`));
          return;
        }
      }
      ...
      if (codeConstraints.length > 1) {
        this._errors.push(new FHIRExportError(`${profile.id}: Found more than one code to fix on ${snapshotEl.id}.  This should never happen and is probably a bug in the tool.`));
      }
    }
  }

  applyIncludesCodeConstraints(sourceValue, profile, snapshotEl, differentialEl) {
    // TODO: This requires slicing.  Yuck.  See: https://chat.fhir.org/#narrow/stream/implementers/subject/fixedUri
  }

  applyBooleanConstraints(sourceValue, profile, snapshotEl, differentialEl) {
    ...
    if (boolConstraints.length > 0) {
      ...
      if (typeof snapshotEl.fixedBoolean !== 'undefined') {
        ...
        // Found another non-matching fixed value.  Put on the brakes.
        if (!disableError.overrideFixedValue) this._errors.push(new FHIRExportError(`${profile.id}: Cannot override fixed boolean on ${snapshotEl.id} from ${snapshotEl.fixedBoolean} to ${value}.`));
        return;
      }
      snapshotEl.fixedBoolean = differentialEl.fixedBoolean = value;
      if (boolConstraints.length > 1) {
        this._errors.push(new FHIRExportError(`${profile.id}: Found more than one boolean to fix on ${snapshotEl.id}.  This should never happen and is probably a bug in the tool.`));
      }
    }
  }
  ...
  addExtension(def, profile, sourcePath, extURL) {
    ...
    const identifier = sourceValue.identifier;
    ...
    this.applyConstraints(sourceValue, profile, ssEl, dfEl);
