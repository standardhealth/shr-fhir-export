...
class FHIRExporter {
  ...
  export() {
    ...
    // (using [x]) is not valid. As a result, we must "fix" the ids in DSTU2.
    // This is still not perfect as it doesn't shorten ids to 64 characters.  I'm not sure
    // how we'd do that and guarantee uniqueness short of just making the id a hash or UUID.
    if (this._target === 'FHIR_DSTU_2') {
      const fixID = (sd, element) => {
        ...
        // Finally, replace unsupported characters with - (for lack of a better approach),
        // but then replace -. and -- with just . or - (since adjacent symbols look odd).
        element.id = element.id.replace(/[^A-Za-z0-9\-\.]/g, '-').replace(/--+/g, '-').replace(/-\.+/g, '.');
      ...
      [...profiles, ...extensions].forEach(sd => {
        sd.snapshot.element.forEach(el => fixID(sd, el));
      });
    }
  ...
  cleanupProfile(profile) {
    ...
    for (const el of profile.snapshot.element) {
      if (el.path.endsWith('[x]') || (el.type && (el.type.length > 1 || this.optionIsSelected(el.type[0])))) {
        ...
        const shrSelected = el.type.reduce((selected, current) => {
          ...
          delete(current._originalProfile);
          delete(current._originalTargetProfile);
          return selected;
        }, { p: 100, types: [] }).types;
    ...
    common.compactStructureDefinition(profile);
  }
...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  findContentReferencePath(profile, snapshotEl) {
    ...
    if (!cRef.startsWith('#')) {
      logger.error('Unsupported contentReference %s on %s (not a local reference). ERROR_CODE:13007', cRef, snapshotEl.id);
      return;
    }
    ...
    if (!crElem) {
      logger.error('Invalid content reference on %s: %s. ERROR_CODE:13008', snapshotEl.id, cRef);
      return;
    }
    return crElem.path;
  }
  ...
  addExplicitChoiceElement(identifier, profile, snapshotEl, differentialEl) {
    ...
    if (typeof identifier === 'string') {
      ...
      if (typeof sdToAdd === 'undefined') {
        logger.error('Cannot make choice element explicit at %s. Invalid identifier: %s. ERROR_CODE:13012', snapshotEl.id, identifier);
        return [];
      }
    ...
    let allowableTargetTypes = this.getTypeHierarchy(sdToAdd.id);
    ...
    if (allowableTargetTypes.length === 0) {
      allowableTargetTypes = this.getTypeHierarchy(MVH.sdType(sdToAdd));
    }
    ...
    if (matchedType == null) {
      logger.error('Cannot make choice element explicit at %s. Could not find compatible type match for: %s. ERROR_CODE:13062', snapshotEl.id, MVH.sdType(sdToAdd));
      return [];
    }
          ...
          if (common.isCustomProfile(sourceProfile)) {
            ...
            if (typeof originalProfile === 'undefined' || allowableTargetProfiles.includes(originalProfile) || basedOnTargetProfiles.includes(originalProfile)) {
              if (sourceValue._derivedFromIncludesTypeConstraint) {
                ...
                if (MVH.typeProfile(t) !== sourceProfile.url) {
      ...
      // @ts-ignore
        ...
        if (this.optionIsSelected(t)) {
          ...
          if (sourceValue._derivedFromIncludesTypeConstraint) {
            ...
            // If the profiles are different, we should represent both the existing and the new profile, so create
            // a new type for the additional profile and splice it into the types
            if (MVH.typeTargetProfile(t) !== sourceProfile.url) {
              const t2 = common.cloneJSON(t);
  ...
  optionIsSelected(option) {
    return option._shrTypePriority ? true : false;
  }
            ...
            closestMatch.elements.forEach(el => {
              ...
              if (targetSS) {
                ...
                logger.error('Could not find FHIR element with path %s for content profile rule with path %s.  ERROR_CODE: 13063',
                  targetPath, cpr.path.map(p => p.name).join('.'));
              }
            });
          } else {
              ...
              if (value) {
                ...
                const isMatchForType = (val, types) => {
                  if (types.length == 1 && types[0].code == 'Extension') {
                    ...
                    if (ext) {
                      return ext.url === MVH.typeProfile(types[0]);
                    }
