...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  unrollContentReference(profile, snapshotEl) {
    ...
    if (!cRef.startsWith('#')) {
      logger.error('Cannot unroll contentReference %s on %s because it is not a local reference. ERROR_CODE:13007', cRef, snapshotEl.id);
      return;
    }
    ...
    const unrolled = [];
    const crPath = MVH.edContentReference(profile, snapshotEl).slice(1);
    let rootId;
    ...
    if (typeof rootId === 'undefined') {
      ...
      logger.error('Invalid content reference on %s: %s. ERROR_CODE:13008', snapshotEl.id, MVH.edContentReference(profile, snapshotEl));
      return;
    }
