...
class FHIRExporter {
  ...
  mappingToProfile(map) {
    ...
    try {
      ...
      profile.name = `${this._config.projectShorthand} ${map.identifier.name} Profile`;
      profile.description = this.getDescription(map.identifier);
      profile.publisher = this._config.publisher;
      profile.contact = this._config.contact;
...
`<div xmlns="http://www.w3.org/1999/xhtml">
    ...
    for (const rule of map.rules) {
      ...
      if (t.hasSliceNumberCommand()) {
        ...
        if (typeof ss === 'undefined') {
          logger.error(`Mapping to ${map.target}'s ${rule.target}: slice could not be found.`);
          continue;
        }
    ...
    for (const t of targets) {
      ...
      if (isRepeatedRoot) {
        ...
        if (typeof ssEl !== 'undefined' && (typeof ssEl.type === 'undefined' || ssEl.type.length < numMappings)) {
          ...
          logger.error('Slicing required to disambiguate multiple mappings to %s. ERROR_CODE:13004', t);
        }
      }
    }
  }
  ...
  processFieldToFieldMappingRule(map, rule, profile) {
    const def = this._specs.dataElements.findByIdentifier(map.identifier);
    if (typeof def === 'undefined') {
      logger.error('Invalid source path. ERROR_CODE:13005');
      return;
    }
    ...
    if (typeof ss === 'undefined' && fieldTarget.target.endsWith('[x]')) {
      ...
      if (sourceValue instanceof mdls.IdentifiableValue) {
        ...
        const map = this._specs.maps.findByTargetAndIdentifier(TARGET, sourceValue.effectiveIdentifier);
        if (typeof map !== 'undefined') {
    ...
    if (typeof ss === 'undefined') {
      logger.error('Invalid or unsupported target path. ERROR_CODE:13006');
      return;
    }
  ...
  unrollContentReference(profile, snapshotEl) {
    if (!snapshotEl.contentReference.startsWith('#')) {
      logger.error('Cannot unroll contentReference %s on %s because it is not a local reference. ERROR_CODE:13007', snapshotEl.contentReference, snapshotEl.id);
      return;
    }
    ...
    if (typeof rootId === 'undefined') {
      ...
      logger.error('Invalid content reference on %s: %s. ERROR_CODE:13008', snapshotEl.id, snapshotEl.contentReference);
      return;
    }
  ...
  unrollElement(identifier, profile, snapshotEl) {
    if (snapshotEl.path.endsWith('[x]')) {
      logger.error('Cannot unroll %s. Create an explicit choice element first. ERROR_CODE:13009', snapshotEl.id);
      return;
    }
    ...
    if (typeof sdToUnroll === 'undefined') {
      logger.error('Couldnt find sd to unroll', identifier, profile.id, snapshotEl.id);
    }
  ...
  processCardinalityMappingRule(map, rule, profile) {
    ...
    if (rule.cardinality.fitsWithinCardinalityOf(targetCard)) {
      ...
      logger.error('Cannot constrain cardinality from %s to %s. ERROR_CODE:13014', targetCard.toString(), rule.cardinality.toString());
    }
  ...
  processFieldToFieldCardinality(map, rule, profile, snapshotEl, differentialEl) {
    ...
    if (targetID.indexOf('.') == targetID.lastIndexOf('.')) {
      ...
      if (aggSourceCard.fitsWithinCardinalityOf(targetCard)) {
        ...
        logger.error('Incompatible cardinality (using aggregation). Source cardinality %s does not fit in target cardinality %s. ERROR_CODE:13017', aggSourceCard.toString(), targetCard);
        return;
      }
    } else {
      ...
      if (aggSourceCard.equals(aggTargetCard)) {
        ...
        if (!isMatch) {
          logger.error('Cannot constrain cardinality to %s because cardinality placement is ambiguous. Explicitly constrain parent elements in target path. ERROR_CODE:13018', aggSourceCard.toString());
          return;
        }
        ...
        if (sMin == aggTargetCard.min) {
          ...
          logger.error('Cannot constrain cardinality to %s because there is no tail cardinality min that can get us there. ERROR_CODE:13019', aggSourceCard.toString());
          return;
        }
        ...
        if (sMax == aggTargetCard.max) {
          ...
          logger.error('Cannot constrain cardinality to %s because there is no tail cardinality max that can get us there. ERROR_CODE:13020', aggSourceCard.toString());
          return;
        }
        ...
        if (magicCard.fitsWithinCardinalityOf(targetCard)) {
          ...
          logger.error('Cannot constrain cardinality to %s because there is no tail cardinality that can get us there. ERROR_CODE:13021', aggSourceCard.toString());
          return;
        }
      }
    }
  ...
  knownMappingIssue(lhs, rhs, sourcePath, value, types) {
    ...
    if (identifier.fqn == lhs || (value && value.identifier && value.identifier.fqn == lhs)) {
      ...
      return types.some(t => t.code == rhs || (profile && t.profile === profile.url)
        || (t.code == 'Reference' && (profile && t.targetProfile == profile.url)));
    }
    return false;
  }
    ...
    if (Array.isArray(allowedConversions[fqn])) {
      for (const type of allowedConversions[fqn]) {
        ...
        const allowedTypes = targetTypes.filter(t => t.code == type
          || (profile && t.profile == profile.url)
          || (profile && t.code == 'Reference' && t.targetProfile == profile.url));
  ...
  getTypeHierarchy(fhirType) {
    ...
    if (typeof type.baseDefinition !== 'undefined') {
      const baseType = this._fhir.find(type.baseDefinition);
      if (baseType) {
  ...
  applyConstraints(sourceValue, profile, snapshotEl, differentialEl, isExtension) {
    ...
    if (sourceValue.constraintsFilter.child.hasConstraints) {
      ...
      if (isExtension) {
        ...
        logger.error('Applying constraints to profiled children not yet supported. SHR doesn\'t have a use case yet. ERROR_CODE:13025');
      }
    }
  }
  ...
  getElementInExtension(shrPath, profile, snapshotEl) {
    ...
    if (isSimple) {
      ...
      const url = common.fhirURL(shrPath[1], this._config.fhirURL, true);
  ...
  applyOwnValueSetConstraints(sourceValue, profile, snapshotEl, differentialEl) {
    ...
    if (vsConstraints.length > 0) {
      ...
      if (vsConstraint.isRequired) {
        ...
        logger.error('Unsupported binding strength: %s. ERROR_CODE:13027', vsConstraint.bindingStrength);
        return;
      }
      ...
      if (bind) {
        if (!allowedBindingStrengthChange(bind.strength, strength)) {
          logger.error('Cannot change binding strength from %s to %s. ERROR_CODE:13028', bind.strength, strength);
          return;
        }
        ...
        if ((bind.valueSetReference && bind.valueSetReference.reference == vsURI) || bind.valueSetUri == vsURI) {
          ...
          if (!this.isValueSetSubsetOfOther(vsURI, bindVSURI)) {
            logger.error('Cannot override value set constraint from %s to %s. ERROR_CODE:13029', bindVSURI, vsURI);
            return;
          }
      ...
      if (vsConstraints.length > 1) {
        logger.error('Found more than one value set to apply to %s. This should never happen and is probably a bug in the tool. ERROR_CODE:13030', snapshotEl.id);
      }
    }
  }
  ...
  fixCodeOnElement(identifier, profile, snapshotEl, differentialEl, code) {
    ...
    case 'code':
      if (snapshotEl.fixedCode && snapshotEl.fixedCode !== code.code) {
        logger.error('Cannot override code constraint from %s to %s', snapshotEl.fixedCode, code.code);
      } else {
    ...
    case 'Quantity': {
      ...
      if (fixedPropName) {
        ...
        if ((fixedX.system && fixedX.system != code.system) || (fixedX.code && fixedX.code != code.code)) {
          logger.error('Cannot override code constraint from %s|%s to %s|%s', fixedX.system, fixedX.code, code.system, code.system);
      ...
      if ((systemEl.fixedUri && systemEl.fixedUri != code.system) || (codeEl.fixedCode && codeEl.fixedCode != code.code)) {
        logger.error('Cannot override code constraint from %s|%s to %s|%s', systemEl.fixedUri, codeEl.fixedCode, code.system, code.system);
        return;
      }
      if (typeof code.system !== 'undefined' && code.system !== null) {
    ...
    case 'CodeableConcept': {
      ...
      if (fixedPropName) {
        ...
        if ((fixedX.coding[0].system && fixedX.coding[0].system != code.system) || (fixedX.coding[0].code && fixedX.coding[0].code != code.code)) {
          logger.error('Cannot override code constraint from %s|%s to %s|%s', fixedX.coding[0].system, fixedX.coding[0].code, code.system, code.system);
      ...
      const sliceEl = {
        ...
        sliceName: sliceName,
        definition: baseCoding.definition,
        min: baseCoding.min, // this is how FHIR vital signs profiles do it
        max: baseCoding.max, // this is how FHIR vital signs profiles do it
  ...
  applyConstraintsForConversion(sourceValue, profile, snapshotEl, differentialEl) {
    ...
    if (sourceValue.constraintsFilter.own.boolean.hasConstraints) {
      ...
      logger.error('Conversion from %s to one of %j drops boolean constraints. ERROR_CODE:13037', sourceIdentifier.fqn, targetTypes);
    } else {
      ...
      if (sourceValue.constraintsFilter.own.valueSet.hasConstraints) {
        logger.error('Conversion from %s to one of %j drops value set constraints. ERROR_CODE:13038', sourceIdentifier.fqn, targetTypes);
      }
      if (sourceValue.constraintsFilter.own.code.hasConstraints) {
        logger.error('Conversion from %s to one of %j drops code constraints. ERROR_CODE:13039', sourceIdentifier.fqn, targetTypes);
      }
      if (sourceValue.constraintsFilter.own.includesCode.hasConstraints) {
        logger.error('Conversion from %s to one of %j drops includesCode constraints. ERROR_CODE:13040', sourceIdentifier.fqn, targetTypes);
      }
    }
  }
  ...
  addExtension(def, profile, sourcePath, extURL) {
    ...
    if (identifier && identifier._namespace === 'unknown') {
      logger.error('Unable to establish namespace for %s. ERROR_CODE:13041', identifier._name);
      return;
    }
    ...
    let ssEl = profile.snapshot.element.find(e => {
      return e.path === `${profile.type}.${extType}` && e.type.length === 1 && e.type[0].profile === extURL;
    });
    ...
    if (typeof ssEl === 'undefined') {
      ...
      if (typeof ssEl.sliceName === 'undefined') {
        dfEl.sliceName = ssEl.sliceName = common.shortID(identifier);
      }
      ...
      if (sourceValue.effectiveCard.fitsWithinCardinalityOf(targetCard)) {
        ...
        logger.error('Cannot constrain cardinality from %s to %s', targetCard.toString(), sourceValue.effectiveCard.toString());
      }
...
function isTargetBasedOn(target, baseTarget) {
  ...
  const [tID, tURL, tType, tBaseDef] = [tDef.id, tDef.url, tDef.type, tDef.baseDefinition];
  ...
  else if ((tID !== tType && isTargetBasedOn(tType, bID)) || (tURL !== tBaseDef && isTargetBasedOn(tBaseDef, bURL))) {
    return true;
  }
