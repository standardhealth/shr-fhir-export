...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  */
  ...
  processContentProfileRules(map, profile) {
    ...
    console.log('CONTENT PROFILE:', cp.identifier.fqn);
    ...
    for (const cpr of cp.rules) {
      ...
      if (closestMatch) {
          ...
          if (targetSubPath) {
            ...
            closestMatch.elements.forEach(el => {
              ...
              if (targetSS) {
                ...
                logger.error('Could not find FHIR element with path %s for content profile rule with path %s.  ERROR_CODE:???',
                ...
                if (value instanceof mdls.ChoiceValue) {
                  closestMatch.elements.forEach(el => {
                    if (value.aggregateOptions.some(opt => isMatchForType(opt, el.type))) {
                      ...
                      logger.error('Could not find FHIR element for content profile rule with path %s.  ERROR_CODE:???',
        ...
        matchedElements.forEach(el => {
          console.log(`  MATCHED ${cprMappingPath}: ${el.id}`);
