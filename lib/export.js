...
`<div xmlns="http://www.w3.org/1999/xhtml">
  ...
  addExtension(map, profile, field) {
    ...
    ssEl.min = field.card.min;
    ssEl.max = typeof field.card.max === 'undefined' ? '*' : field.card.max.toString();
  ...
  pushDefExtensionElements(extension, baseId, baseExpression, def, card=new mdls.Cardinality(0)) {
    ...
    } else if (typeof def.value !== 'undefined' && def.value.card.max <= 1 && def.fields.length == 0) {
      card = def.value.card;
    ...
    if (typeof type === 'undefined') {
      ...
      for (const field of [def.value, ...def.fields]) {
        if (typeof field === 'undefined') {
          ...
          if (field.identifier.isPrimitive) {
            ...
            fieldDef = new mdls.DataElement(field.identifier, false);
            fieldDef.description = def.description;
            fieldDef.value = new mdls.IdentifiableValue(field.identifier).withMinMax(1, 1);
          } else {
            fieldDef = this._specs.dataElements.findByIdentifier(field.identifier);
          }
          const fieldBaseId = `${baseId}.extension:${shortID(fieldDef.identifier)}`;
          const fieldBaseExpression = this.appendExtensionExpression(baseExpression, fieldDef);
          this.pushDefExtensionElements(extension, fieldBaseId, fieldBaseExpression, fieldDef, field.card);
        } else if (field instanceof mdls.ChoiceValue) {
          // For now... we put the choice options at the same level as everything else, but we might want to nest
          // them in the future
          ...
          for (const opt of this.aggregateChoiceOptions(field)) {
            if (opt instanceof mdls.TBD) {
              ...
              // Ugh.  Look at all this duplicated code
              let optDef;
              if (opt.identifier.isPrimitive) {
                // This can only be in the value... so hijack the def and ovverride it's identifier with this one
                // TODO: Is there a better way?  Should we actually use the real primitive's definitions?
                optDef = new mdls.DataElement(opt.identifier, false);
                optDef.description = def.description;
                optDef.value = new mdls.IdentifiableValue(opt.identifier).withMinMax(1, 1);
              } else {
                optDef = this._specs.dataElements.findByIdentifier(opt.identifier);
              }
              const optBaseId = `${baseId}.extension:${shortID(optDef.identifier)}`;
              const optBaseExpression = this.appendExtensionExpression(baseExpression, optDef);
              const optCard = new mdls.Cardinality(0, field.card.max);
              this.pushDefExtensionElements(extension, optBaseId, optBaseExpression, optDef, optCard);

              optExpressions.push(optBaseExpression);
            }
          }
          if (optExpressions.length > 1) {
            ...
            for (let i=0; i < optExpressions.length; i++) {
              if (i > 0) {
                constraint.human += ' or ';
                constraint.expression += ' xor ';
              }
              constraint.human += optExpressions[i];
              constraint.expression += `${optExpressions[i]}.exists()`;
            }
      ...
      this.pushNoValueXElement(extension, baseId);
    } else {
      this.pushExtensionNoExtensionsElement(extension, baseId);
      this.pushExtensionURLElement(extension, baseId, def);
      this.pushValueXElement(extension, baseId, type, card, profileIdentifier, choice);
    }
  }

  appendExtensionExpression(baseExpression, def) {
    if (baseExpression.length > 0) {
      return `${baseExpression}.extension('${shortID(def.identifier)}')`;
    }
    return `extension('${shortID(def.identifier)}')`;
  }
  ...
  pushValueXElement(extension, baseId, type, card=new mdls.Cardinality(1,1), profileIdentifier, choice) {
  ...
  pushNoValueXElement(extension, baseId) {
    const ssValue = {
      id: `${baseId}.value[x]`,
