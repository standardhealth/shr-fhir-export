...
class ExtensionExporter {
  ...
  pushDefExtensionElements(extension, baseId, baseExpression, def, card=new mdls.Cardinality(0)) {
    ...
    if (typeof type === 'undefined') {
      ...
      this.pushExtensionURLElement(extension, baseId, def);
      this.pushExtensionValueXElement(extension, baseId, def, type, value);
    }
  }
  ...
  pushExtensionValueXElement(extension, baseId, def, type, value) {
    ...
    if (value instanceof mdls.ChoiceValue) {
      for (const opt of value.aggregateOptions) {
        ...
        if (typeof t === 'undefined') {
          continue;
        } else if (ssValue.type.some(x => x.code == t.code && (MVH.typeProfile(x) && (MVH.typeProfile(x).some(tp => MVH.typeHasProfile(t, tp))) || (MVH.typeTargetProfile(x) && MVH.typeTargetProfile(x).some(tp => MVH.typeHasTargetProfile(t, tp)))))) {
          ...
          if (t.profile && t.profile.length > 0) {
            t.profile.forEach(p => MVH.addTypeProfile(extension, ssValue.type, t.code, p));
          } else if (t.targetProfile && t.targetProfile.length > 0) {
            t.targetProfile.forEach(tp => MVH.addTypeTargetProfile(extension, ssValue.type, t.code, tp));
          } else {
            if (!ssValue.type.some(vt => vt.code === t.code && vt.profile == null && vt.targetProfile == null)) {
              ssValue.type.push(t);
            }
          }
    ...
    } else if (value && value instanceof mdls.IdentifiableValue && ! value.identifier.isPrimitive) {
      ...
      if (type == 'Reference') {
        MVH.addTypeTargetProfile(extension, ssValue.type, type, profile.url);
      } else if (common.isCustomProfile(profile)) {
        MVH.addTypeProfile(extension, ssValue.type, type, profile.url);
      } else {
        if (!ssValue.type.some(t => t.code === type && t.profile == null && t.targetProfile == null)) {
    ...
    if (cp != null) {
      ...
      cpRules.forEach(r => {
        if (r.path.length === 1) {
          if (r.mustSupport) {
            ssValue.mustSupport = dfValue.mustSupport = true;
          }
        } else {
          ...
          if (targetSubPath) {
            ...
            const targetPath = `${ssValue.path}.${targetSubPath}`;
  ...
  pushExtensionNoValueXElement(extension, baseId) {
    const ssValue = {
      ...
      max: '0',
      base: { path: 'Extension.value[x]', min: 0, max: '1' },
      // TODO: Probably should (finally) fix this by getting these from the FHIR definitions?
      type: [
  ...
  pushExtensionSubExtensionElements(extension, baseId, baseExpression, def, field) {
    ...
    if (field.identifier && field.identifier._namespace === 'unknown') {
      logger.error('Unable to establish namespace for %s. ERROR_CODE:13045', field.identifier._name);
      return;
    }
    ...
    const definition = field.identifier.isPrimitive ? (def.description ? def.description.trim() : undefined) : subExt.description
    const ssExt = {
      id: `${fieldBaseId}`,
      ...
      isModifierReason: isModifier ? definition : undefined,
      mapping: [{ identity: 'rim', map: 'N/A' }]
    };
    if (typeof definition === 'undefined' || definition == null || definition == '') {
      // definition is *required*, so put something there
      ssExt.definition = field.identifier.name;
      // isModifierReason is required (R4) is isModifier is set
      if (isModifier) {
        ssExt.isModifierReason = `${field.identifier.name} modifies the meaning of the thing to which it is applied`;
      }
    }
    ...
    const dfExt = {
      ...
      max: ssExt.max,
      type: ssExt.type,
      isModifier: ssExt.isModifier ? ssExt.isModifier : undefined,
      isModifierReason: ssExt.isModifierReason
    };
