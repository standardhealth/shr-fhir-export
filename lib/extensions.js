...
class ExtensionExporter {
  ...
  createExtension(identifier) {
    ...
    // 03019, 'Start creating extension',,
    logger.debug('03019');
  ...
  setValueXElement(extension, def, type, value) {
    ...
    for (const invType of invalidTypes) {
      // 13114, 'Cannot create extension with value[x] type: ${type}', 'Check value to ensure it is a primitive, maps to a FHIR datatype, or is an Entry (which allows references)', 'errorNumber'
      logger.error({ type: JSON.stringify(invType) }, '13114');
      ...
      cpRules.forEach(r => {
        if (r.path.length === 1) {
          ...
          if (targetSubPath) {
            ...
            logger.error({ path: r.path.map(p => p.name).join('.') }, '13064');
          }
        }
      });
    }
  ...
  pushExtensionSubExtensionElements(extension, baseId, def, field) {
    ...
    if (field.effectiveIdentifier && field.effectiveIdentifier._namespace === 'unknown') {
      ...
      logger.error({field1 : field.effectiveIdentifier.name},'13045' );
      return;
    }
    ...
    if (value && value instanceof mdls.IdentifiableValue) {
      if (value.effectiveIdentifier.isPrimitive) {
        ...
        if (value2 == null) {
          ...
          logger.error({ elementId: value.effectiveIdentifier }, '13115');
          return;
        }
