    ...
    if (cp != null) {
      ...
      cpRules.forEach(r => {
        if (r.path.length === 1) {
	      ...
	      //logger.error('Could not find FHIR element with path %s for content profile rule with path %s.  ERROR_CODE:13063', targetPath, r.path.map(p => p.name).join('.'));
	      //13063, 'Could not find FHIR element with ${path1} %s for content profile rule with ${path2}',  'Unknown' , 'errorNumber'
	      logger.error({ path1 : targetPath, path2:  r.path.map(p => p.name).join('.') }, '13063' );
    ...
    if (cp != null) {
      ...
      cpRules.forEach(r => {
        if (r.path.length === 1) {
	      ...
	      //logger.error('Could not find FHIR element subextension for content profile rule with path %s.  ERROR_CODE:13065',  r.path.map(p => p.name).join('.'));
	    //13065, 'Could not find FHIR element subextension for content profile rule with path ${path}' ,  'Unknown' , 'errorNumber'
	    logger.error({ path :  r.path.map(p => p.name).join('.') }, '13065' );
          }
        }
      });
    }
