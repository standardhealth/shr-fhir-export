...
class ExtensionExporter {
  ...
  pushDefExtensionElements(extension, baseId, baseExpression, def, card=new mdls.Cardinality(0)) {
    ...
    if (cp != null) {
      console.log('EXT CONTENT PROFILE:', cp.identifier.fqn);
    }
  ...
  pushExtensionValueXElement(extension, baseId, def, type, value) {
    ...
    if (cp != null) {
      ...
      cpRules.forEach(r => {
        if (r.path.length === 1) {
          console.log(`  MATCHED ${r.path.map(id => id.name).join('.')}: ${ssValue.id}`);
          if (r.mustSupport) {
          ...
          if (targetSubPath) {
            ...
            if (targetSS) {
              console.log(`  MATCHED ${r.path.map(id => id.name).join('.')}: ${targetSS.id}`);
              if (r.mustSupport) {
  ...
  pushExtensionSubExtensionElements(extension, baseId, baseExpression, def, field) {
    ...
    if (cp != null) {
      ...
      cpRules.forEach(r => {
        if (r.path.length === 1) {
          console.log(`  MATCHED ${r.path.map(id => id.name).join('.')}: ${ssExt.id}`);
          if (r.mustSupport) {
          ...
          if (targetSS) {
            ...
            logger.error('Could not find FHIR element subextension for content profile rule with path %s.  ERROR_CODE:???',
              r.path.map(p => p.name).join('.'));
          }
        }
      });
