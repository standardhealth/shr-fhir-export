...
class ExtensionExporter {
  ...
  createExtension(identifier) {
    ...
    try {
      ...
      // Start from the Extension definition in the spec
      const baseExt = this._fhir.findType('Extension');
      // Add one key at a time to insert missing elements at proper locations (aesthetic, but nice)
      const ext = {};
      for (const key of Object.keys(baseExt)) {
        if (key === 'type' && baseExt.contextType === undefined) {
          // contextType comes before context (STU3 only, but will delete later)
          ext.contextType = '';
        }
        if (key === 'type' && baseExt.context === undefined) {
          // context comes before type
          ext.context = [];
        }
        ...
        if (key === 'url' && baseExt.identifier === undefined) {
          // identifier comes after url
          ext.identifier = [];
        } else if (key === 'name' && baseExt.title === undefined) {
          // title comes after name
          ext.title = '';
        } else if (key === 'kind' && baseExt.constrainedType === undefined) {
          // constrainedType comes after kind
          ext.constrainedType = '';
        }
      }

      // First modify all the necessary things regardless of the specific extension data
      delete(ext.meta);
      ...
      if (this._target === 'FHIR_DSTU_2') {
        delete ext.title;
        ext.base = ext.url;
        ext.constrainedType = 'Extension';
        ext.contextType = 'resource';
        ext.context = ['*'];
      } else if (this._target === 'FHIR_STU_3') {
        ext.baseDefinition = ext.url;
        delete ext.constrainedType;
        ...
        delete ext.constrainedType;
        ext.derivation = 'constraint';
        delete ext.contextType;
        ext.context = [{ type: 'element', expression: '*' }];
      }
      ext.differential = { element: []};
      const baseId = `Extension:${common.shortID(def.identifier)}`;
      for (const ssEl of ext.snapshot.element) {
        ssEl.id = ssEl.id.replace(new RegExp(/^[^\.]+/m), baseId);
        delete(ssEl.extension);
      }

      // Then modify data specific to *this* extension
  ...
  getTypeFromValue(value, originalTypes) {
    if (value && value instanceof mdls.IdentifiableValue) {
      if (value.effectiveIdentifier.isPrimitive) {
        ...
        const type = value.effectiveIdentifier.name;
        return { code: type === 'xhtml' ? 'string' : type };
      } else {
