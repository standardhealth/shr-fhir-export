...
class ExtensionExporter {
  ...
  createExtension(identifier) {
    ...
    ext.url = common.fhirURL(def.identifier, this._config.fhirURL, true);

    ext.name = ext.title = `${this._config.projectShorthand} ${def.identifier.name} Extension`;
    ext.publisher = this._config.publisher;
    ext.contact = this._config.contact;
  ...
  pushExtensionURLElement(extension, baseId, def) {
    const ssURL = {
      ...
      type: [{ code: 'uri' }],
      fixedUri: baseId.indexOf('.') == -1 ? common.fhirURL(def.identifier, this._config.fhirURL, true) : common.shortID(def.identifier),
      mapping: [{ identity: 'rim', map: 'N/A' }]
    };
