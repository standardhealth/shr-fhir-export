...
class ValueSetExporter {
  ...
  export(profiles, models, extensions) {
    let usedValueSets = [];
    // get all value sets from profile snapshots
    // not all value sets are relevant, but this
    // is fine for our filtering purposes
    for (const profile of profiles) {
      if (profile.snapshot.element != null) {
        for (const element of profile.snapshot.element) {
          if (element.binding && element.binding.valueSetReference) {
            usedValueSets.push(element.binding.valueSetReference.reference);
          }
        }
      }
    }

    // get all value sets from model snapshots
    // not all value sets are relevant, but this
    // is fine for our filtering purposes
    for (const model of models) {
      if (model.snapshot.element != null) {
        for (const element of model.snapshot.element) {
          if (element.binding && element.binding.valueSetReference) {
            usedValueSets.push(element.binding.valueSetReference.reference);
          }
        }
    ...
    // get all value sets from extension snapshots
    // not all value sets are relevant, but this
    // is fine for our filtering purposes
    for (const extension of extensions) {
      if (extension.snapshot.element != null) {
    ...
    // if filtering, only export used value sets
    let filter = false;
