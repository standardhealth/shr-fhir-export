...
function getFHIRElement(targetItem, elements, target, validateChoice=true) {
  ...
  for (const el of elements) {
    if (el.path == path) {
      if (validateChoice && el.path.endsWith('[x]') && typeof choice != 'undefined') {
...
module.exports = {FHIRExportError, getSnapshotElement, getDifferentialElement, getFHIRElement, addSlicingToBaseElement, createSlicingObject, elementTypeContainsTypeName, fhirID, fhirURL, shortID, valueAndFields, equalShrElementPaths, escapeHTML, cloneJSON};