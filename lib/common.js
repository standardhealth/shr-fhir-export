...
const shortenedIdMap = new Map();
function fhirID(identifier, extra = '') {
  const genID = (prefix, name, postfix) => {
    const id = prefix ? `${prefix}-${name}` : name;
    return postfix ? `${id}-${postfix}` : id;
  };
  let id = genID(identifier.namespace.replace(/\./g, '-'), identifier.name, extra);
  // FHIR imposes an id length of 64 characters, which the IG publisher is now enforcing
  // See: http://hl7.org/fhir/R4/datatypes.html#id
  if (id.length > 64) {
    // If we've seen this before and shortened it, return the shortened id
    if (shortenedIdMap.has(id)) {
      return shortenedIdMap.get(id);
    }
    // We need to shorten it.  First try removing the namespace.
    const oldId = id;
    id = genID('', identifier.name, extra);
    if (id.length > 64) {
      // It's still too long; just truncate the name to the length we need.
      id = genID('', identifier.name.slice(0, identifier.name.length - (id.length - 64)), extra);
      // Now check and adjust in case something else was already shortened to this
      for (let i=2; Array.from(shortenedIdMap.values()).indexOf(id) !== -1; i++) {
        // replace the last character(s) of the name with an incrementing number until we get something unique
        id = genID('', identifier.name.slice(0, identifier.name.length - (id.length - 64 - i.toString().length) + i), extra);
      }
    }
    shortenedIdMap.set(oldId, id);
  }
  return id;
}
 ...
 * - or if there is a subset constraint that contains the id
 * @param {Object} value - the value to get the effective identifier from
 */
function choiceFriendlyEffectiveIdentifier(value, id=null) {
  ...
  // If subset constraints, check if id is contained in the subset, if it is
  // return the type that matches id
  const ownSubsetConstraints = value.constraintsFilter.own.subset.constraints;
  if (value instanceof mdls.ChoiceValue && ownSubsetConstraints.length === 1 && id != null) {
    const match = ownSubsetConstraints[0].subsetList.filter(type => type.equals(id));
    if (match.length === 1) {
      return match[0];
    }
  }
...
function subsetEffectiveIdentifiers(value) {
  const ownSubsetConstraints = value.constraintsFilter.own.subset.constraints;
  if (value instanceof mdls.ChoiceValue && ownSubsetConstraints.length === 1) {
    return ownSubsetConstraints[0].subsetList;
  }
}
 ...
 * @param {Object} baseSD - the base StructureDefinition of the one to compact.  This is needed to determine what can safely
 *   be removed from the snapshot.
 */
function compactStructureDefinition(sd, baseSD) {
  ...
  // are represented in the snapshot of the base structuredefinition nor the differential of this one.
  // Note -- if their parent is a BackboneElement, however, then do not filter them out.
  ...
  sd.snapshot.element = sd.snapshot.element.filter((el) => {
    ...
    // If it has children in its base structure definition snapshot, keep it and don't mark it to be compacted
    if (baseSD && baseSD.snapshot.element.some(ss => ss.id.startsWith(`${el.id}.`))) {
      return true;
    }
    ...
    // Otherwise, it's not a BackboneElement and has no children in the base snapshot or profile differential,
    // so keep it, but mark it as a compacted element
...
/**
 * Checks if the mapping has, or inherits from a mapping that has, a no profile (NP)
 * flag in the content profile file.
 * @param {Object} map - The map that is being checked for a NP flag.
 * @param {Object} contentProfiles - The contentProfiles object from the _specs.
 */
function hasContentProfileNoProfile(map, contentProfiles) {
  let cp;
  // Get content profile of map, or parent of map
  if (map == null) {
    return false;
  }
  else if (map.inheritance === 'inherited') {
    cp = contentProfiles.findByIdentifier(map.inheritedFrom);
  } else {
    cp = contentProfiles.findByIdentifier(map.identifier);
  }
  // If contentProfile exists, iterate through rules and check for no profile
  if (cp != null) {
    return cp.rules.some(rule => rule.noProfile);
  }
  return false;
}
