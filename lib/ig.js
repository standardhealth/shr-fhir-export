...
${match[1]}
  ...
  // Copy over the US Core definitions file.  This is needed due to a bug in the published US-Core.
  // See: https://chat.fhir.org/#narrow/stream/implementers/topic/Publisher.20broken.20when.20using.20.20uscore.20dependency.3F
  fs.copySync(path.join(__dirname, 'definitions', 'FHIR_STU_3', 'IGs', 'US_Core', 'definitions.json.zip'),
    path.join(outDir, 'uscore', 'definitions.json.zip'));
  ...
  if (!config.igLogicalModels) {
    ...
    fs.writeFileSync(navbarPath, navbarHtml.replace(modelsItem, '<!-- no logical models -->'));
  }
}
