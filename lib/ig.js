...
function exportIG(fhirResults, outDir, configuration = [], specPath) {
  ...
  try {
    ...
    igIndexContent =
`<p> This is an FHIR implementation guide for the ${config.projectName}. </p>`;
  ...
  fs.writeFileSync(igIndexHtmlPath, igIndexHtml.replace(/<project-name-go-here>/g, config.projectName)
                                               .replace(/<project-url-go-here>/g, config.projectURL)  
                                               .replace("<igIndexContent-go-here>", igIndexContent)                                                 
