...
function exportIG(specifications, fhirResults, outDir, configuration = {}, specPath) {
  ...
  for (const valueSet of fhirResults.valueSets) {
    if (valueSet.url.startsWith(config.projectURL)) {
...
${match[1]}
  ...
  const sortedPrimaryExternalValueSetUrls = Array.from(primaryExternalValueSetUrls).sort(byValue);
  for (const url of sortedPrimaryExternalValueSetUrls) {
