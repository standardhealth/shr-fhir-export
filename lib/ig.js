...
function exportIG(specifications, fhirResults, outDir, configuration = {}, specPath) {
  ...
  // Override includeLogicalModels if not STU3 (since not supported in DSTU2/R4)
  if (target !== 'FHIR_STU_3' && config.implementationGuide) {
  ...
  // HL7 IGs have special publication rules:
  // - They must specify a history link
  // - They must have a package-list.json (see: http://wiki.hl7.org/index.php?title=FHIR_IG_PackageList_doco).
  if (igControl.canonicalBase && /^https?:\/\/([^\/]+\.)?(hl7|fhir)\.org\//.test(igControl.canonicalBase)) {

    // If the config specifies a history link, use it.  Otherwise, algorithmically generate it
    if (config.implementationGuide && config.implementationGuide.historyLink) {
      igControl.paths.history = config.implementationGuide.historyLink;
    } else {
      igControl.paths.history = `${config.fhirURL}/history.html`;
    }

    // Now get or create the package-list.json
    let packageList;
    // Look for it in the root folder.  If it doesn't exist, create a new one from the information we have.
    const packageListFile = path.join(specPath, 'package-list.json');
    if (fs.existsSync(packageListFile)) {
      ...
      logger.warn(`HL7 IGs require a package-list.json file.  A starter version has been written to ${packageListFile}. ` +
        'For more information on package files, see: http://wiki.hl7.org/index.php?title=FHIR_IG_PackageList_doco');
    }

    // Check the package list for obvious issues
    if (packageList['package-id'] !== igControl['npm-name']) {
      logger.error(`Package-id '${packageList['package-id']}' (found in package-list.json) does not match npm-name '${igControl['npm-name']}' from config file`);
    }
    if (packageList['package-id'] !== igControl['npm-name']) {
      logger.error(`Canonical URL '${packageList['canonical']}' (found in package-list.json) does not match canonical URL '${igControl['canonicalBase']}' from config file`);
    }
    if (!packageList.list || packageList.list.length === 0) {
      logger.error('The package-list.json file must have at least one listed publication object');
    } else {
      const ci = packageList.list[0];
      if (ci.version !== 'current' || ci.status !== 'ci-build' || ci.current !== true) {
        logger.error('The first publication object in the package-list.json list must be the CI build (with "version":"curent", "status":"ci-build", and "current":true');
      }
      if (!ci.path || ci.path.endsWith('xxx')) {
        logger.warn(`The package-list.json contains an invalid path for the CI build: ${ci.path}`);
      }
    }
...
${match[1]}
  ...
  const filterFunc = (src) => {
    ...
    if (src.endsWith('.json')) {
      logger.warn(`WARNING: Overwriting generated profile with patch: ${src}`);
      return true;
    }
    return false;
  };
  ...
  if (config.implementationGuide.examples) {
    ...
    if (!fs.existsSync(igExamplesPath)) {
      logger.error(`Specified examples path is not valid: ${igExamplesPath}`);
      igExamplesPath = null;
    }
...
function removeSHRMappings(sd) {
  ...
  common.compactStructureDefinition(sd);
