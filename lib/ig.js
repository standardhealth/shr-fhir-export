...
function exportIG(fhirResults, outDir) {
  ...
  // 4. Add it to the resources or extensions HTML file
  ...
  for (const profile of fhirResults.profiles.sort(sdCompare)) {
    ...
    const name = profile.name.replace(/^Standard Health Record /, '').replace(/ Profile$/, '');
    htmlResources.push(`<tr>
          <td>${namespace}</td>
  ...
  for (const extension of fhirResults.extensions.sort(sdCompare)) {
    ...
    const name = extension.name.replace(/^Standard Health Record /, '').replace(/ Extension$/, '');
  ...
  // Rewrite the updated Resources HTML file
  const resourcesHtmlPath = path.join(outDir, 'pages', '_includes', 'resources.html');
  const resourcesHtml = fs.readFileSync(resourcesHtmlPath, 'utf8');
  fs.writeFileSync(resourcesHtmlPath, resourcesHtml.replace('<resources-go-here/>', htmlResources.join('')));

  // Rewrite the updated Extensions HTML file
  const extensionsHtmlPath = path.join(outDir, 'pages', '_includes', 'extensions.html');
