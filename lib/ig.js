...
`<p> This is a ${config.projectName} FHIR implementation guide. </p>`;
  ...
  for (const profile of fhirResults.profiles.sort()) {
    ...
    const mapPath = path.join(outDir, 'pages', '_includes', `${profile.id}-${config.projectShorthand.toLowerCase()}-mapping.xhtml`);
...
${match[1]}
  ...
  for (const extension of fhirResults.extensions.sort()) {
    ...
    const mapPath = path.join(outDir, 'pages', '_includes', `${extension.id}-${config.projectShorthand.toLowerCase()}-mapping.xhtml`);
    fs.writeFileSync(mapPath, '', 'utf8');
  }
