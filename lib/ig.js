...
`<p> This is a ${config.projectName} FHIR implementation guide. </p>`;
  ...
  for (const profile of fhirResults.profiles) {
    ...
    xmlResources.push(`<resource>
        ...
        `);
    const namespace = getNamespaceFromID(profile.id, config);
...
${match[1]}
  ...
  for (const extension of fhirResults.extensions) {
    ...
    xmlResources.push(`<resource>
        ...
        `);
    const namespace = getNamespaceFromID(extension.id, config);
  ...
  for (const valueSet of fhirResults.valueSets) {
    ...
    const namespace = getNamespaceFromID(valueSet.id, config);
  ...
  for (const codeSystem of fhirResults.codeSystems) {
    ...
    const namespace = getNamespaceFromID(codeSystem.id, config);
  ...
  fhirResults.models.sort(byID);
  for (const model of fhirResults.models) {
    fs.writeFileSync(path.join(sdPath, `${model.id}.json`), JSON.stringify(model, null, 2), 'utf8');
    ...
    xmlResources.push(`<resource>
        ...
        `);
    const namespace = getNamespaceFromID(model.id);

    const name = model.name.replace(new RegExp('^' + config.projectShorthand + ' '), '').replace(/ Logical Model$/, '');
    htmlModels.push(
    `<tr>
      <td>${namespace}</td>
  ...
  const igXmlPath = path.join(outDir, 'resources', 'ImplementationGuide', 'ig.xml');
  ...
  const modelsHtml = fs.readFileSync(profilesHtmlPath, 'utf8');
  fs.writeFileSync(modelsHtmlPath, modelsHtml.replace('<resources-go-here/>', htmlModels.join(''))
                                             .replace('<project-shorthand-go-here/>', config.projectShorthand), 'utf8');
}
