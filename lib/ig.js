...
function exportIG(specifications, fhirResults, outDir, configuration = {}, specPath) {
  ...
  if (target === 'FHIR_DSTU_2') {
    ...
    //   name : 'argonaut',
    //   location : 'http://www.fhir.org/guides/argonaut/r2',
    //   version: '1.0.0'
    // }];
  }
...
${match[1]}
  ...
  */
  ...
  // Add missing images and rewrite the updated Downloads HTML file (DSTU2 only)
  if (target === 'FHIR_DSTU_2') {
    // Copy over the images that the FHIR IG publisher fails to include (for some reason)
    fs.copySync(path.join(__dirname, 'ig_files', 'missing_dstu2_images'), path.join(outDir, 'pages'));

    // Remove references to TTL downloads since the IG publisher doesn't create a TTL downloads file for DSTU2
