...
${match[1]}
  ...
  if (config.implementationGuide.extraResources) {
    ...
    if (!fs.existsSync(igExtraResourcesPath)) {
      logger.error(`Specified extraResources path is not valid: ${igExtraResourcesPath}`);
    ...
    for (const rscFile of fs.readdirSync(igExtraResourcesPath)) {
      ...
      try {
        ...
        if (!exResource.resourceType || !exResource.id) {
          logger.error('Invalid extra resource. Extra resource JSON must include id and resourceType properties: %s', rscFilePath);
          continue;
        } else if (exResource.fhirVersion && exResource.fhirVersion !== igControl.version) {
          logger.error('Invalid extra resource. IG is for FHIR %s, but resource if for FHIR %s: %s', exResource.fhirVersion, igControl.version, rscFilePath);
          continue;
        }
        ...
        case 'Conformance':
          ...
          break;
        default:
          logger.error('Invalid extra resource. Only the following resource types are currently supported: StructureDefinition, ValueSet, CodeSystem, SearchParameter, OperationDefinition, CapabilityStatement, Conformance.  Found: %s', exResource.resourceType);
          continue;
        }
        ...
        logger.error('Invalid extra resource.  Resource must be valid JSON: %s', rscFilePath);
      }
    }
  }
