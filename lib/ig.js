...
function exportIG(specifications, fhirResults, outDir, configuration = {}, specPath) {
  ...
  const target = specifications.maps.targets.length ? specifications.maps.targets[0] : 'FHIR_STU_3';
  ...
  const pushXmlResource = (reference, display, isExample=false) => {
    if (target === 'FHIR_DSTU_2') {
      // No example tag (introduced in STU3)
      // TODO: Populate ExampleFor
  ...
  for (const profile of fhirResults.profiles) {
    ...
    pushXmlResource(`StructureDefinition/${profile.id}`, MVH.sdTitle(profile));
...
${match[1]}
  ...
  for (const extension of fhirResults.extensions.sort(byName)) {
    ...
    pushXmlResource(`StructureDefinition/${extension.id}`, MVH.sdTitle(extension));
  ...
  for (const valueSet of fhirResults.valueSets) {
    ...
    pushXmlResource(`ValueSet/${valueSet.id}`, MVH.vsTitle(valueSet));
  ...
  if (target !== 'FHIR_DSTU_2') {
    ...
    for (const codeSystem of fhirResults.codeSystems) {
      ...
      pushXmlResource(`CodeSystem/${codeSystem.id}`, codeSystem.title);
  ...
  if (config.implementationGuide.includeLogicalModels) {
    ...
    for (const model of fhirResults.models) {
      ...
      pushXmlResource(`StructureDefinition/${model.id}`, model.name);
  ...
  if (fs.existsSync(fhirExamplesPath)) {
    ...
    for (const exFile of fs.readdirSync(fhirExamplesPath)) {
      ...
      try {
        ...
        pushXmlResource(`${example.resourceType}/${example.id}`, `Example: ${example.id}`, true);
      } catch (e) {
  ...
  if (target === 'FHIR_DSTU_2') {
    ...
    const downloadsHtml = fs.readFileSync(codeSystemsHtmlPath, 'utf8');
