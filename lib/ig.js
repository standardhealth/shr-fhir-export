...
${match[1]}
  ...
  let sortedExternalValueSetUrls;
  if (hideSupporting) {
    sortedExternalValueSetUrls = Array.from(primaryExternalValueSetUrls).sort(byValue);
  } else {
    sortedExternalValueSetUrls = Array.from(externalValueSetUrls).sort(byValue);
  }

  for (const url of sortedExternalValueSetUrls) {
