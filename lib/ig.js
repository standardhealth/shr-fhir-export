...
function exportIG(specifications, fhirResults, outDir, configuration = {}, specPath) {
  ...
  const isHL7IG = igControl.canonicalBase && /^https?:\/\/([^\/]+\.)?(hl7|fhir)\.org\//.test(igControl.canonicalBase);

  // HL7 IGs must have a history link
  if (isHL7IG) {
  ...
  let packageListFile;
  if (config.implementationGuide && config.implementationGuide.packageList) {
